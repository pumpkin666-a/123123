 ...
 *
 * @help 使你可以在：
 * 1.穿戴装备时：打开开关、变量增加/减少。脱下装备时：关闭开关、变量减少/增加。
 * 2.获得状态时：打开开关、变量增加/减少。移除状态时：关闭开关、变量减少/增加。
 * 3.可以设置每个角色独立的开关和变量。
 *   开启专属后，原指定的通用变量和开关不再生效。
 *   独立开关/变量ID=通用开关/变量ID+角色ID
 *   [举例]通用变量设置#10，则2号角色专用变量ID = 通用ID+角色ID = 10 + 2 = 12
 * 4.当指定角色离开队伍时，其指定的开关会关闭。
 *
 * 【注0】本插件需要PluginCommonBase.js支持，并在列表中置于PluginCommonBase之下。
 *
 * 【注1】
 * 通常情况下，你可以指定 武器/防具/状态的ID来执行通用变量/开关的操作。
 * 插件也支持"追加"指定武器类型/防具类型/装备类型。但....

 * [举例]指定：长剑（武器ID）变量+100，剑类（武器类型）变量+50。
 * 1.当你装备长剑时，变量+150。（满足：武器ID+武器类型ID）
 * 2.但如果只装备短剑，则变量一点都不增加。（满足：武器类型ID，但不满足：武器ID）
 * **很奇怪的效果，不知道是不是BUG，或者我没搞对用法，请自行测试~

 * 【注2】
 * 操作列表内的一个设置，请只指定一种武器或防具或状态的ID，多选无效。
 * 
 *
 * 使用条款：
 * 1.没有任何使用限制，可以修改和二次发布，不要求声明出处，作者不对使用后果负责。
 * 2.这个插件现在是你的了。
 */
 ...
 * @param name
 * @text 识别名称
 * @desc 用于在插件设置中显示的名称，没有特别用途。
 *
 * @param weaponId
 * @text 武器ID
 * @desc 指定一个武器，穿戴/脱下时会改变变量和开关。
 * @type weapon
 * @default 0
 *
 * @param armorId
 * @text 防具ID
 * @desc 指定一个防具，穿戴/脱下时会改变变量和开关。
 * @type armor
 * @default 0
 *
 * @param stateId
 * @text 状态ID
 * @desc 指定一个状态，获得/解除时会改变变量和开关。
 * @type state
 * @default 0
 *
 * @param equipType
 * @text 追加装备类型
 * @desc 追加指定一种装备类型，可搭配对应的武器、防具ID使用。
 * @type number
 * @default 0
 *
 * @param weaponType
 * @text 追加武器类型
 * @desc 追加指定一种武器类型，可搭配对应的武器、防具ID使用。
 * @type number
 * @default 0
 *
 * @param armorType
 * @text 追加防具类型
 * @desc 追加指定一种防具类型，可搭配对应的武器、防具ID使用。
 * @type number
 * @default 0
 *
 * @param list
 * @text 变量开关列表
 * @desc 指定会根据装备或状态而变化的变量或开关。
 ...
 * @param addActorId
 * @text 角色专属变量/开关
 * @desc Ture-每个角色使用专用的变量或开关ID。开启后通用变量/开关不再生效。角色独立ID=通用ID+角色ID
 * @type boolean
 * @default false
 *
 * @param operand
 * @text 变量变化值
 * @desc 指定变量的变动值。（范围-99999到99999）
 * @type number
 * @default 0
 * @min -99999
 * @max 99999
